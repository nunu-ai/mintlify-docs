...
openapi: 'POST /project/{projectId}/test-plans'
noindex: true
---
    ...
    <ParamField body="test_plan.test_items" type="TestPlanTestItem[]" required>
      ...
      <Expandable title="test_item properties">
        ...
        <ParamField body="retries" type="integer" default="0">
          Per-item retry count (0 or higher). Retries apply to execution failures (`ERROR`, `TIMEOUT`, `STOPPED`); genuine test verdicts (`FAILED`, `BLOCKED`, `SKIPPED`) are final and are not retried.
        </ParamField>
      </Expandable>
    </ParamField>
  </Expandable>
